...
export default class VertexBuffer {
     ...
     * @private
     */
    constructor(context: Context, array: StructArray, attributes: StructArrayMember[], dynamicDraw?: boolean) {
    ...
    setVertexAttribPointers(gl: WebGLRenderingContext, program: Program, vertexOffset?: number) {
        for (let j = 0; j < this.attributes.length; j++) {
            const member = this.attributes[j];
            const attribIndex: number = program.attributes[member.name];
